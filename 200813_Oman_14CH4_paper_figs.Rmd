...
MWL_plot <- data_water_isotopes %>% ggplot() +
  ...
  # axis styling
  scale_x_continuous(limits = c(-5, 1), name = latex2exp::TeX("$\\delta^{18}O\\, /\\, \\[$‰$  \\, VSMOW \\]$")) +
  scale_y_continuous(limits = c(-25, 10), name = latex2exp::TeX("$\\delta D\\, /\\, \\[$‰$  \\, VSMOW \\]$")) +
...
CD_plot <-
  ...
  # axis styling
  scale_y_continuous(name = latex2exp::TeX("$\\delta D_{CH_4}\\,/\\,\\[$‰$  \\, VSMOW \\]$"), limits = c(-450, 0), expand = c(0,0)) +
  scale_x_continuous(name = latex2exp::TeX("$\\delta ^{13}C_{CH_4}\\,/\\,\\[$‰$  \\, VPDB \\]$"), limits = c(-90, 15), expand = c(0,0)) +
...
```{r}
data_14CH4_2019_gas_concs <- data %>% filter(year_sampled == 2019) %>% filter(sampling_site == "NSHQ14" | sampling_site == "WAB71") %>% select(sampling_site, notes, CH4_uM, CH4_LQ_uM, C2H6_uM, C2H6_LQ_uM, C3H8_uM, C3H8_LQ_uM, i_C4H10_uM, i_C4H10_LQ_uM, n_C4H10_uM, n_C4H10_LQ_uM, i_C5H12_uM, i_C5H12_LQ_uM, C6H14_uM, C6H12_LQ_uM)

data_14CH4_2019_gas_concs
```
calculate equilibrium gas phase concentration
```{r}
data_14CH4_2019_gas_concs_gas_phase <- data_14CH4_2019_gas_concs %>% mutate(CH4_uM = CH4_uM / Henrys %>% filter(compound == "methane") %>% select(Hcc_22C) %>% as.numeric)
