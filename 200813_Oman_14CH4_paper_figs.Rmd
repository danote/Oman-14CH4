...
data_CD_gathered <- full_join(data_C_gathered, data_D_gathered) %>% filter(sampling_site != "WAB56")
```
...
alkanes_d13C_oman_wells_and_lit_plot <- ggplot() +
  ...
  # Add NSHQ14 label
  annotate("label", label = "2017", x = 1.17, y = 1.3, label.size = 0.25) +
  annotate("label", label = "2019", x = 1.17, y = 8.3, label.size = 0.25) +
...
```{r get-14CH4-data}
# create data frame of CH4, C2H6, and C3H8 d13C data
data_14CH4 <- data %>% filter(!is.na(CH4_Fm_UCIAMS)) %>% select(sampling_site, year_sampled, notes, CH4_Fm_UCIAMS, CH4_Fm_unc_UCIAMS, CH4_Fm_NOSAMS, CH4_Fm_unc_NOSAMS, CH4_d13C_permil_VPDB_CUB, CH4_d13C_unc_permil_CUB, CH4_d13C_permil_VPDB_ISOTECH, CH4_d13C_unc_permil_ISOTECH)
...
str_detect(parameter, "UCIAMS") == TRUE ~ "INSTAAR/UCIAMS",
str_detect(parameter, "NOSAMS") == TRUE ~ "Isotech/NOSAMS"
))  %>%
  # drop "parameter column
  select(-parameter) %>% 
  # assign Fm uncertainties based on lab
  mutate(CH4_Fm_unc = if_else(lab == "Isotech/NOSAMS", CH4_Fm_unc_NOSAMS, CH4_Fm_unc_UCIAMS)) %>% 
  ...
  mutate(CH4_d13C_permil_VPDB = if_else(lab == "Isotech/NOSAMS", CH4_d13C_permil_VPDB_ISOTECH, CH4_d13C_permil_VPDB_CUB)) %>% 
  select(-CH4_d13C_permil_VPDB_ISOTECH, -CH4_d13C_permil_VPDB_CUB) %>%
  # assign CH4 d13C uncertainty based on lab
    mutate(CH4_d13C_unc_permil = if_else(lab == "Isotech/NOSAMS", CH4_d13C_unc_permil_ISOTECH, CH4_d13C_unc_permil_CUB)) %>% 
...
```{r}
# create plot
data_14CH4_NSHQ14_longer %>% ggplot(
  aes(
    x = CH4_Fm,
    xmin = CH4_Fm - CH4_Fm_unc,
    xmax = CH4_Fm + CH4_Fm_unc,
    y = CH4_d13C_permil_VPDB,
    ymin = CH4_d13C_permil_VPDB - CH4_d13C_unc_permil,
    ymax = CH4_d13C_permil_VPDB + CH4_d13C_unc_permil,
    shape = as.character(year_sampled),
    color = lab,
    label = notes
  )
  ...
  geom_label(nudge_x = 0.045, ) +
  
  scale_x_continuous(name = "Fraction modern", limits = c(0, 0.32), expand = c(0,0)) +
...
```{r}
data_C1_C2_C3 %>% filter(sampling_site == "NSHQ14") %>% select(sampling_site, year_sampled, C1_over_C2_plus_C3, notes)
```
