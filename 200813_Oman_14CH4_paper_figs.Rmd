...
```{r}
# data_ASF_LCHF_mol_frac_summ %>% select(contains("mol_frac_mean"))
```
...
# data_ASF_LCHF_mol_frac_longer_stderr <- data_ASF_LCHF_mol_frac_summ %>% pivot_longer(
#         cols = contains("stderr"),
#   names_to = "C_number",
#   values_to = "mol_frac_stderr"
# ) %>% mutate(
#   C_number = C_number %>% str_sub(2,2) %>% as.numeric()
# ) %>% select(C_number, mol_frac_stderr)
# 
# data_ASF_LCHF_mol_frac_longer_and_stderr <- data_ASF_LCHF_mol_frac_longer %>% left_join(data_ASF_LCHF_mol_frac_longer_stderr, by = "C_number")

# take log10 of mol_frac and stderr
data_ASF_LCHF_mol_frac_longer <-data_ASF_LCHF_mol_frac_longer %>%
  mutate(log10_mol_frac = log10(mol_frac)) # take log10
    # mutate(stderr_over_mean_percent = mol_frac_stderr / mol_frac * 100) # take log10
