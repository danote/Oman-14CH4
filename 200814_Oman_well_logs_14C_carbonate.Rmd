...
BA3A_NSHQ14_profile_longer_Eh_T_P <- BA3A_NSHQ14_profile_longer_Eh_T_P %>%
  mutate(
    parameter = case_when(
        ...
        parameter == "H2O_H2_limit_Eh_f_T_P_mV" ~ "Stability limit of water with respect to molecular hydrogen as a function of temperature, pressure, and pH"
    )
  )
```
...
plot_BA3A_NSHQ14_Eh <- BA3A_NSHQ14_profile_longer_Eh_T_P  %>% ggplot(aes(x = depth_sampled_mbgl))+
  ...
  scale_y_continuous(name = "Eh / [mV]")+
  scale_color_manual(name = "Parameter", values = c("Measured" = "black", "Stability limit of water with respect to molecular hydrogen as a function of T and P" = "#D55E00")) +
